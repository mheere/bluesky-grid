...
import __de = require('./DataEngine');
import __cm = require('./CurrencyManager');

// Blue Sky Grid Controller - 
export class GridController {

    public dataEngine: __de.DataEngine = null;
    ...
    private $datarows: JQuery = undefined;
    ...
    private _selectedPKvalue: string = "";
    ...
    // --------------------------------
    public setData(data, colDefinitions?: ColDefinition[]): JQueryPromise<string> {
        ...
        if (!self.dataEngine) {
            ...
            var coldef: ColDefinition = self.colDefinitions.findItem(function (cdl: ColDefinition) {
    ...
    private processDataRows(): JQueryPromise<string> {
    ...
    private refreshPage(action: __de.NavigationAction) {
    ...
    public resized() {
        ...
        setTimeout(function () {
            ...
            if (newwidth == -1) return;         // there is no flex column
    ...
    private _isResizingColumn: boolean = false;
    private _isDraggingColumn: boolean = false;
    ...
    private attachDelegatedHandlers() {
        ...
        $(".btnFirstPage", self.$pager).click(function () { self.refreshPage(__de.NavigationAction.first); });
        $(".btnPrevPage", self.$pager).click(function () { self.refreshPage(__de.NavigationAction.prev); });
        $(".btnNextPage", self.$pager).click(function () { self.refreshPage(__de.NavigationAction.next); });
        $(".btnLastPage", self.$pager).click(function () { self.refreshPage(__de.NavigationAction.last); });
        ...
        $grid.on("mouseenter", ".cell-header", function () {
            ...
            if (self._isResizingColumn || self._isDraggingColumn) return;
